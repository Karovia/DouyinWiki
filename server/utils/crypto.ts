...
let cachedBaseSecret: Buffer | null = null;

function getCachedBaseSecret(): Buffer {
  if (!cachedBaseSecret) {
    cachedBaseSecret = getBaseSecret();
  }
  return cachedBaseSecret;
}

function deriveKey(salt: Buffer): Buffer {
  const baseSecret = getCachedBaseSecret();
  return scryptSync(baseSecret, salt, 32);
}
